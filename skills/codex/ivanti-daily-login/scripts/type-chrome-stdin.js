...
function run(argv) {
  ...
  if (
    ...
    frontmostProcesses[0].bundleIdentifier() !== chromeBundleIdentifier
  ) {
    value = "";
    throw new Error("Chrome is not the frontmost application");
  }
  ...
  try {
    ...
    value = "";
    throw new Error("No focused Chrome accessibility element");
  }
  ...
  if (role !== "AXTextField" && subrole !== "AXSecureTextField") {
    value = "";
    throw new Error("Focused Chrome element is not a text field");
  }
  ...
  if (!expectedLabels.some((label) => labels.includes(label))) {
    value = "";
    throw new Error(`Focused Chrome field is not ${expectedField}`);
  }
  ...
  try {
    ...
    value = "";
    throw new Error("Unable to set focused Chrome field");
  }
  value = "";
  return "";
}
